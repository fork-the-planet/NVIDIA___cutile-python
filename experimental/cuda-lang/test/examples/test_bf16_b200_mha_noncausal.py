...
def probability_vector(values, scale, offset):
    p0 = fast_exp2(cl.bitcast(values[0], cl.float32) * scale + offset)
    p1 = fast_exp2(cl.bitcast(values[1], cl.float32) * scale + offset)
    p2 = fast_exp2(cl.bitcast(values[2], cl.float32) * scale + offset)
    p3 = fast_exp2(cl.bitcast(values[3], cl.float32) * scale + offset)
    p4 = fast_exp2(cl.bitcast(values[4], cl.float32) * scale + offset)
    p5 = fast_exp2(cl.bitcast(values[5], cl.float32) * scale + offset)
    p6 = fast_exp2(cl.bitcast(values[6], cl.float32) * scale + offset)
    p7 = fast_exp2(cl.bitcast(values[7], cl.float32) * scale + offset)
    p8 = fast_exp2(cl.bitcast(values[8], cl.float32) * scale + offset)
    p9 = fast_exp2(cl.bitcast(values[9], cl.float32) * scale + offset)
    p10 = fast_exp2(cl.bitcast(values[10], cl.float32) * scale + offset)
    p11 = fast_exp2(cl.bitcast(values[11], cl.float32) * scale + offset)
    p12 = fast_exp2(cl.bitcast(values[12], cl.float32) * scale + offset)
    p13 = fast_exp2(cl.bitcast(values[13], cl.float32) * scale + offset)
    p14 = fast_exp2(cl.bitcast(values[14], cl.float32) * scale + offset)
    p15 = fast_exp2(cl.bitcast(values[15], cl.float32) * scale + offset)
    p16 = fast_exp2(cl.bitcast(values[16], cl.float32) * scale + offset)
    p17 = fast_exp2(cl.bitcast(values[17], cl.float32) * scale + offset)
    p18 = fast_exp2(cl.bitcast(values[18], cl.float32) * scale + offset)
    p19 = fast_exp2(cl.bitcast(values[19], cl.float32) * scale + offset)
    p20 = fast_exp2(cl.bitcast(values[20], cl.float32) * scale + offset)
    p21 = fast_exp2(cl.bitcast(values[21], cl.float32) * scale + offset)
    p22 = fast_exp2(cl.bitcast(values[22], cl.float32) * scale + offset)
    p23 = fast_exp2(cl.bitcast(values[23], cl.float32) * scale + offset)
    p24 = fast_exp2(cl.bitcast(values[24], cl.float32) * scale + offset)
    p25 = fast_exp2(cl.bitcast(values[25], cl.float32) * scale + offset)
    p26 = fast_exp2(cl.bitcast(values[26], cl.float32) * scale + offset)
    p27 = fast_exp2(cl.bitcast(values[27], cl.float32) * scale + offset)
    p28 = fast_exp2(cl.bitcast(values[28], cl.float32) * scale + offset)
    p29 = fast_exp2(cl.bitcast(values[29], cl.float32) * scale + offset)
    p30 = fast_exp2(cl.bitcast(values[30], cl.float32) * scale + offset)
    p31 = fast_exp2(cl.bitcast(values[31], cl.float32) * scale + offset)

    packed = cl.Vector(
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p1, p0), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p3, p2), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p5, p4), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p7, p6), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p9, p8), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p11, p10), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p13, p12), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p15, p14), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p17, p16), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p19, p18), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p21, p20), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p23, p22), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p25, p24), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p27, p26), cl.int32),
        cl.bitcast(cl._nvvm.ff2bf16x2_rn(p29, p28), cl.int32),
...
def scale_vector16(values, scale):
    return cl.Vector(
        cl.bitcast(cl.bitcast(values[0], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[1], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[2], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[3], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[4], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[5], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[6], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[7], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[8], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[9], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[10], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[11], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[12], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[13], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[14], cl.float32) * scale, cl.int32),
        cl.bitcast(cl.bitcast(values[15], cl.float32) * scale, cl.int32),
    )
        ...
        while current_bid < total_bids:
            ...
            for key_block in range(iterations):
                ...
                scores = cl.tcgen05_load(
                    cl.Tcgen05LoadStoreShape.SHAPE_32X32B, row_tmem, count=32
                )
                ...
                scores = cl.tcgen05_load(
                    ...
                    cl.tcgen05_tmem_offset(row_tmem, column_offset=32),
                    count=32,
                )
                cl.tcgen05_wait_load()
                row_max = max_vector32(scores, row_max)
                scores = cl.tcgen05_load(
                    cl.Tcgen05LoadStoreShape.SHAPE_32X32B,
                    cl.tcgen05_tmem_offset(row_tmem, column_offset=64),
                    count=32,
                )
                cl.tcgen05_wait_load()
                row_max = max_vector32(scores, row_max)
                scores = cl.tcgen05_load(
                    cl.Tcgen05LoadStoreShape.SHAPE_32X32B,
                    cl.tcgen05_tmem_offset(row_tmem, column_offset=96),
                    count=32,
                )
                cl.tcgen05_wait_load()
                row_max = max_vector32(scores, row_max)
                ...
                # Keeping all four quarters live, as CUDA does, currently adds
                # 624 stack bytes per thread; CUDA compiles spill-free. Reload
                # until cuda.lang's backend matches that register allocation.
                scores = cl.tcgen05_load(
                    cl.Tcgen05LoadStoreShape.SHAPE_32X32B, row_tmem, count=32
                )
                cl.tcgen05_wait_load()
                probabilities, block_sum = probability_vector(scores, scale, offset)
                cl.tcgen05_store(
                    cl.Tcgen05LoadStoreShape.SHAPE_32X32B, row_tmem, probabilities
                )
                ...
                cl.tcgen05_wait_load()
                probabilities, quarter_sum = probability_vector(scores, scale, offset)
                block_sum += quarter_sum
                cl.tcgen05_store(
                    cl.Tcgen05LoadStoreShape.SHAPE_32X32B,
                    cl.tcgen05_tmem_offset(row_tmem, column_offset=16),
                    probabilities,
                )
                cl.tcgen05_wait_store()
                if cl.elect_sync():
                    cl.mbarrier_arrive(
                        cl.map_shared_to_cluster(
                            norm_quarter_arrived.get_element_pointer((0, qid)), 0
                        )
                    )
                scores = cl.tcgen05_load(
                    cl.Tcgen05LoadStoreShape.SHAPE_32X32B,
                ...
                if cl.elect_sync():
                    cl.mbarrier_arrive(
                        cl.map_shared_to_cluster(
                            norm_quarter_arrived.get_element_pointer((1, qid)), 0
                        )
                    )
                scores = cl.tcgen05_load(
                    cl.Tcgen05LoadStoreShape.SHAPE_32X32B,
                ...
                cl.tcgen05_store(
                    cl.Tcgen05LoadStoreShape.SHAPE_32X32B,
                    cl.tcgen05_tmem_offset(row_tmem, column_offset=48),
                    probabilities,
                )
                cl.tcgen05_wait_store()
                if cl.elect_sync():
                    cl.mbarrier_arrive(
                        cl.map_shared_to_cluster(
                            norm_quarter_arrived.get_element_pointer((2, qid)), 0
                        )
                    )
